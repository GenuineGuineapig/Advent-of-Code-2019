...
public class firstHalf {
    ...
    private static final HashMap<Coordinate, Coordinate> map = new HashMap<>();

    private static Coordinate walk(String s, int xStart, int yStart, int line) {
        int way = Integer.parseInt(s.substring(1));
        int x = xStart;
        int y = yStart;
        ...
        switch(s.charAt(0)) {
            case 'R':
                for (int i = 0; i < way; i++) {
                    Coordinate c = new Coordinate(--x, y, line);
                    if (map.get(c) != null && map.get(c).w.ordinal() != line) {
        ...
        return new Coordinate(x, y, line);
    }
    ...
    public static void main(String[] args) {
        ...
        for (String s : split) {
            ...
            for (String c : commands) {
                Coordinate walk = walk(c, x, y, line);
                x = walk.x;
                y = walk.y;
            }
            line++;
        ...
        System.out.println(map
                .values()
                .stream()
                .filter(c -> c.w == Wire.BOTH)
                .mapToInt(c -> Math.abs(c.x) + Math.abs(c.y))
                .min()
                .getAsInt());
